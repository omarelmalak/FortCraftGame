...
public class AdventureGame implements Serializable {
    ...
    private final String[] actionVerbs = {"QUIT","INVENTORY","TAKE","DROP","WEAPONS", "LOAD", "POTIONS", "KEYS"}; //List of action verbs (other than motions) that exist in all games. Motion vary depending on the room and game.
    public Player player; //The Player of the game.
    private ArrayList<Troll> activeTrolls;
    /**
    ...
    public AdventureGame(String name){
        ...
        this.activeTrolls = new ArrayList<>();
    ...
    public void setUpGame() throws IOException {
        ...
        // set up the player's current location (SINGLETON)
        this.player = Player.getInstance();
        this.player.setInventory();
        this.player.setHealth(100);
        this.player.setCurrentRoom(this.rooms.get(1));
    ...
    public String interpretAction(String command){
        ...
        if (motionTable.optionExists(inputArray[0])) {
            ...
            if(inputArray[0].equals("QUIT")) { return "GAME OVER"; } //time to stop!
            ...
            else if(inputArray[0].equals("WEAPONS") && this.player.getWeapons().size() == 0) return "YOU HAVE NO WEAPONS";
            else if(inputArray[0].equals("WEAPONS") && this.player.getWeapons().size() > 0) return "YOU HAVE THESE WEAPONS:\n" + this.player.getWeaponsStringFormat().toString();
            else if(inputArray[0].equals("POTIONS") && this.player.getPotions().size() == 0) return "YOU HAVE NO POTIONS";
            else if(inputArray[0].equals("POTIONS") && this.player.getPotions().size() > 0) return "YOU HAVE THESE POTIONS:\n" + this.player.getPotionsStringFormat().toString();
            else if(inputArray[0].equals("KEYS") && this.player.getKeys().size() == 0) return "YOU HAVE NO KEYS";
            else if(inputArray[0].equals("KEYS") && this.player.getKeys().size() > 0) return "YOU HAVE THESE KEYS:\n" + this.player.getKeysStringFormat().toString();
            ...
            else if(inputArray[0].equals("LOAD") && inputArray.length < 2) return "THE LOAD COMMAND REQUIRES THE NAME OF THE SAVE";
            else if(inputArray[0].equals("LOAD") && inputArray.length > 2) return "THE LOAD COMMAND TAKES IN ONLY ONE ARGUMENT";
            else if(inputArray[0].equals("TAKE") && inputArray.length >= 2) {
                String objectString = "";
                for (String word : Arrays.copyOfRange(inputArray, 1, inputArray.length)) {
                    objectString += word + " ";
                }
                objectString = objectString.substring(0, objectString.length() - 1);
                if(this.player.getCurrentRoom().checkIfObjectInRoom(objectString)) {
                    this.player.takeObject(objectString);
                ...
                if (objectString.toLowerCase().equals("chug jug") || objectString.toLowerCase().equals("med kit") || objectString.toLowerCase().equals("bandages")) {
                    return "YOU CANNOT DROP A POTION";
                }

                if(this.player.checkIfObjectInInventory(objectString)) {
                    this.player.dropObject(objectString);
     ...
     * getActiveTrolls
     * __________________________
     * Getter method for activeTrolls
     */
    public ArrayList<Troll> getActiveTrolls() {
        return this.activeTrolls;
    }
